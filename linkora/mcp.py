        ...
        return json.dumps(
            {
                ...
                "install_hint": "pip install linkora[embed]",
            }
        )
    except FileNotFoundError:
        return json.dumps(
            {"error": "vectors_not_found", "message": "Run: linkora embed"}
        )
    except Exception as e:
        return json.dumps({"error": "internal", "message": str(e)})
...
def main():
    """Entry point for linkora-mcp command."""
    mcp.run()
